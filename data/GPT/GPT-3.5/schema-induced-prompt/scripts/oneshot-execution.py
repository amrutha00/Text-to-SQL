...
if db_populated > 0: 
    zeroshot_path = "/workspace/data/cs598-tpcds/data/gpt/100/gpt-schema-one-shot"
    os.makedirs(zeroshot_path, exist_ok=True)
    ...
    #indices = random.sample(range(1, 100), 33)
    count_fail = 0
    for i in [42]:#range(77,100):
        print(f"Executing query {i}")
        sql_file_path = os.path.join(gpt_model,f"queries/oneshot-sqlquery{i}.sql")
